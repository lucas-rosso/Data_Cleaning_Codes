/****************************************************************************************************
* Append and clean data from the survey of consumer finances (SCF)
* Author: Lucas Rosso
...
if "$user" == "LR" global path_init = "C:\Users\LR\Desktop\Lucas Rosso\lucas-rosso.github.io\resources_LR\Data_resources\SCF"
...
gen educ= (X5904==1) if !missing(X5904) // college degree
...
replace IRA_safe=  2*( X3610 + X3620 + X3630 )/3 if X3631==4 // combination of bank accounts, stocks and bonds so we give 2/3 to safe
replace IRA_safe=  ( X3610 + X3620 + X3630 ) /2 if inlist(X3631, 5,6,-7) // give half to safe if both and half to risky
...
replace annuity_safe=  (X6820)/2 if inlist(X6826, 5,6,-7) // give half to safe if both and half to risky
...
gen owner = net_house_wealth>0 & !missing(net_house_wealth)
* --------------------------------------
...
gen IRA_safe = (X3610 + X3620 + X3630)  if inlist(X3631, 1,3,11)     
replace IRA_safe=0 if IRA_safe==. 
replace IRA_safe=  2*( X3610 + X3620 + X3630 )/3 if X3631==4 // combination of bank accounts, stocks and bonds, we give 2/3 to safe
replace IRA_safe=  ( X3610 + X3620 + X3630 ) /2 if inlist(X3631, 5,6,-7) // give half to safe if both and half to risky        
...
******************************************************************************** 
	...
	if `var' == 04 | `var' == 07 | `var' == 10 | `var' == 13 {
		...
		gen educ = (X5931 > 9) if !missing(X5931)
	}
...
********************************************************************************
	...
	* --------------------------------------
	...
	save "$clean/scf`var'.dta", replace
}
...
use "$clean/scf98", clear

local survey_periods = "01 04 07 10 13 16 19"

foreach var in `survey_periods' {
	append using "$clean/scf`var'.dta"
}

mer m:1 year using `cpi', nogen
...
********************************************************************************
*  EDITING DATA AND GENERATING VARIABLES (NEW DEFINITION)
********************************************************************************

/* Baseline definition includes retirement accounts for financial wealth so we
rename safe and risky asset variables. */
...
qui forval i = 1/100 {
	* Illiquid Asset
	...
	sum part_Illiquid_assets_hoseH [aw=wgt] if FW_percentileH == `i', detail
...
*** SAVING DATA ***
save "$clean/SCF_Data.dta", replace
